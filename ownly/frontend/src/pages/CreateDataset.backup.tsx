...
import { Search, Loader2, ArrowRight, Zap, Check, X } from 'lucide-react';

import { DomainSuggestion, SelectedFields, Source } from '../types';
      ...
      <div className="main-content">
        {isLoading ? (
                ...
                />
                      ...
                      <div className="space-y-4">
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                          <h3 className="text-sm font-medium text-slate-900 mb-2">Your API Endpoint</h3>
                          <div className="flex items-center">
                            <code className="flex-1 bg-white px-3 py-2 rounded text-sm font-mono text-slate-800 overflow-x-auto">
                              https://api.ownly.com/v1/datasets/preview?token=demo_123456
                            </code>
                            <button
                              onClick={() => {
                                navigator.clipboard.writeText('https://api.ownly.com/v1/datasets/preview?token=demo_123456');
                                toast.success('Copied to clipboard!');
                              }}
                              className="ml-2 px-3 py-2 bg-white border border-slate-200 rounded-md text-sm font-medium text-slate-700 hover:bg-slate-50"
                            >
                              Copy
                            </button>
                          </div>
          ...
          <motion.div 
            ...
            className="text-center py-16 px-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border-2 border-dashed border-blue-200"
          >
            ...
            <div className="space-y-3">
              <button
                type="button"
                onClick={() => {
                  setQuery("connect italian language learning to airbnb rentals in italy");
                  setTimeout(() => {
                    const form = document.querySelector("form");
                    if (form) form.requestSubmit();
                  }, 0);
                }}
                className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-sm font-medium rounded-xl hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all transform hover:scale-105 shadow-md"
              >
                <span>Try our demo: "Connect Italian learning to Airbnb in Italy"</span>
