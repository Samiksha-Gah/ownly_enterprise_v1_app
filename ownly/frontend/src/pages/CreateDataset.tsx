...
const DEMO_DATA = {
  query: "connect language learning to italian airbnb rentals",
  ...
  estimated_monthly_cost: Math.round((12473 * 0.0015 * 30) * 100) / 100 // users * avg_price_per_user_day * days_in_month
};
...
export default function CreateDataset() {
  ...
  const handleSearch = useCallback((e: React.FormEvent) => {
    ...
    // Simulate API call
    setTimeout(() => {
      // For demo purposes, we'll use the same mock data for any query
      // In a real app, this would be an API call with the actual query
      setDetectedDomains(DEMO_DATA.detected_domains);
      
      // Generate some variation based on the query
      const queryLength = query.length;
      const randomFactor = (queryLength % 10) / 10 + 0.8; // 0.8 to 1.8 multiplier
      
      setEstimatedUsers(Math.round(DEMO_DATA.estimated_users * randomFactor));
      setEstimatedMonthlyCost(Math.round(DEMO_DATA.estimated_monthly_cost * randomFactor * 100) / 100);
  ...
  useEffect(() => {
    setQuery('');
    // Don't preload any data until search is performed
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="mb-10 text-center">
        ...
        <button
          type="button"
          onClick={() => {
            setQuery("connect language learning to italian airbnb rentals");
            const form = document.querySelector('form');
            if (form) form.requestSubmit();
          }}
          className="mt-6 inline-flex items-center px-6 py-3 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
        >
          <span>Try our demo: "Connect language learning to Italian Airbnb rentals"</span>
